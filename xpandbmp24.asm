...
%define     denominator   	[ebp+16]		; points to sclae den
...
; esi used for inline pointer
; ebx used to remember the store remainder for additional lines
; eax used to store int(scale)
; ecx used as temp
; edx used to remember how much remainder is
; edi used as picture pointer
...
xpandbmp24:
    ...
    div     dword denominator     			; eax = edx:eax / denominator
    lea     eax,    [eax*2 + eax + 3]
    ...
    mov     ebx,	 0           			; this will be increased by remider if its > denominator it means its time to add pixel
...
clonepixel:
    ...
    mov     ecx,    denominator             		; moving denumentor to register to optimize algoritm

    add     ebx, 	edx          			; increment reminder counter
    cmp     ebx, 	ecx   			        ; compares reminder with scale denominator
    jb      nextpixel	    		        	; if remider < denominator we go to next
    
    sub     ebx, 	ecx                     	; remider >= denominator we add another pixel and decrement the counter
...
cloneremainder:
    ...
    cmp     ebx, 	ecx				
    jb      nextline				        ; ebx < denominator no need to copy 
    
    sub     ebx, 	ecx			        ; ebx >= copy one more line same procedure as above
    mov     ecx, 	bnwidth
    shr     ecx, 	2
